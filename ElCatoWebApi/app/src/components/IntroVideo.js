...
export default function IntroVideo(props) {
    ...
    useEffect(() => {
        ...
        quoteElRef.current.innerText = quotes[randomNum];
        ...
        <>
            <div ref={mainDivRef} className="fixed top-0 left-0 w-full h-full pt-36 text-center bg-base-100 z-40">
                <div className="fixed top-[45%] left-1/2 -translate-x-1/2 -translate-y-1/2 z-30">
                    ...
                    <p ref={quoteElRef} className="mt-10 mx-auto !text-center"></p>
                </div>
            </div>
            <p ref={skipElRef} hidden={!videoStarted} className="fixed bottom-[10%] left-1/2 -translate-x-1/2 -translate-y-1/2 z-[101] text-warning">
